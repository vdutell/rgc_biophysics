...
def create_stim(morph_fname, soma_size_multiplier=1, dend_size_multiplier=1):
    ...
    #print(my_cell.axon)
    my_cell = utils.RGC(morph_fname, 0, soma_size_multiplier, dend_size_multiplier)
...
midget_t_p83, midget_v_p83 = create_stim(midget_fname, soma_size_multiplier=0.833, dend_size_multiplier=0.833)
midget_t, midget_v = create_stim(midget_fname, soma_size_multiplier=1, dend_size_multiplier=1)
#midget_t_1p1, midget_v_1p1 = create_stim(midget_fname,soma_size_multiplier=1.1, dend_size_multiplier=1.1)
midget_t_1p2, midget_v_1p2 = create_stim(midget_fname,soma_size_multiplier=1.2, dend_size_multiplier=1.2)
#midget_t_1p5, midget_v_1p5 = create_stim(midget_fname,soma_size_multiplier=1.5, dend_size_multiplier=1.5)
...
parasol_t_p83, parasol_v_p83 = create_stim(parasol_fname, soma_size_multiplier=0.833, dend_size_multiplier=0.833)
parasol_t, parasol_v = create_stim(parasol_fname, soma_size_multiplier=1, dend_size_multiplier=1)
#parasol_t_1p1, parasol_v_1p1 = create_stim(parasol_fname,soma_size_multiplier=1.1, dend_size_multiplier=1.1)
parasol_t_1p2, parasol_v_1p2 = create_stim(parasol_fname,soma_size_multiplier=1.2, dend_size_multiplier=1.2)
#parasol_t_1p5, parasol_v_1p5 = create_stim(parasol_fname,soma_size_multiplier=1.5, dend_size_multiplier=1.5)
...
plt.plot(midget_t_p83, midget_v_p83, label='midget soma+dend 0.833x')
plt.plot(midget_t, midget_v, label='midget soma+dend 1x')
#plt.plot(midget_t_1p1, midget_v_1p1, label='soma+dend 1.1x')
plt.plot(midget_t_1p2, midget_v_1p2, label='midget soma+dend 1.2x')
#plt.plot(midget_t_1p5, midget_v_1p5, label='soma+dend 1.5x')
...
plt.subplot(3,1,3)
plt.plot(parasol_t_p83, parasol_v_p83, label='parasol soma+dend 0.833x')
plt.plot(parasol_t, parasol_v, label='parasol soma+dend 1x')
#plt.plot(parasol_t_1p1, parasol_v_1p1, label='soma+dend 1.1x')
plt.plot(parasol_t_1p2, parasol_v_1p2, label='parasol soma+dend 1.2x')
#plt.plot(parasol_t_1p5, parasol_v_1p5, label='soma+dend 1.5x')
plt.xlabel('Time (ms)')
plt.ylabel('Soma Voltage (mV)')
plt.title(f'Parasol Response to Sine Wave at {stim_fq_hz} Hz, {stim_fq_amp} A')
plt.legend()
plt.tight_layout()
plt.savefig(f'soma_voltage_{stim_fq_hz}hz_{stim_fq_amp}Amps_somadendtest.png')
